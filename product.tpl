		...
		<div class="pb-center-column col-xs-12 col-sm-6"> 
			...
			<div class="referencesmm {if file_exists($manpic)}extraPadd{/if}">
				...
				<p id="product_reference"{if empty($product->reference) || !$product->reference} style="display: none;"{/if}>
					...
					{/if}
					<meta itemprop="brand" content="{if $product->manufacturer_name}{$product->manufacturer_name}{else}Massageboutik{/if}" />
	...
	{/if}
			...
			<form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
				...
				{assign var="mm_SpecificProducts" value=[179,180,195]}
					...
					{if in_array($product->id,$mm_SpecificProducts)}
				...
				<div class="box-info-product">
					...
					<div class="addtocartbox"><div class="quantity">	
					 	...
					 	{if isset($HOOK_PRODUCT_ACTIONS) && $HOOK_PRODUCT_ACTIONS}{$HOOK_PRODUCT_ACTIONS}{/if}</div> <!-- end box-cart-bottom -->
